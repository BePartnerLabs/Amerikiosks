...
export default function SeedPage() {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', maxWidth: 520, margin: '4rem auto', padding: '0 1.5rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <button onClick={() => run()} style={btnStyle('#011936', '#fff')}>
          🌱 Seed everything{badge('__all__')}
        </button>
      </div>

      <hr style={{ margin: '1.5rem 0', borderColor: '#eee' }} />
      <p style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: '#999', marginBottom: '1rem' }}>
        Individual sections
      </p>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {PARTS.map(({ key, label }) => (
          <button key={key} onClick={() => run(key)} style={btnStyle('#f4f4f5', '#111')}>
            {label}{badge(key)}
          </button>
        ))}
      </div>
