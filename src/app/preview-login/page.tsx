  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '300px' }}>
        ...
        <p style={{ margin: 0, color: '#666', fontSize: '0.9rem' }}>Enter the access password to continue.</p>
        <input
          type="password"
          value={password}
          ...
          autoFocus
          style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '6px', fontSize: '1rem' }}
        />
        {error && <p style={{ color: 'red', margin: 0, fontSize: '0.85rem' }}>Incorrect password</p>}
        <button type="submit" style={{ padding: '8px 12px', background: '#000', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '1rem' }}>
          Access Site
        </button>
      </form>
    </div>
