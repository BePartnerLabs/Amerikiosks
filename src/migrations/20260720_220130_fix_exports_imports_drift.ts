...
// Hand-written from the SQL Drizzle originally generated (and this repo's
// migrate:create bundled together with the unrelated Claims refund-field
// change in 20260720_214945_add_claims_refund_fields — deliberately split
// out, see that file's comment). These tables/enum values were orphaned
// remnants of Payload's import/export feature — confirmed unused (0 rows in
// both exports and imports locally) and confirmed with the team before
// generating this migration. Payload's config has never defined these
// collections in this schema version, so the Drizzle snapshot already treats
// them as absent; this migration exists purely to bring the physical database
// in line with that already-tracked schema state — `migrate:create` reports
// "no schema changes detected" for this and will continue to do so, since as
// far as the tracked snapshot chain is concerned nothing further has changed.
