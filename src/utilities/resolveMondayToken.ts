 ...
 *
 * So in development a MONDAY_API_TOKEN in .env.local wins: that is where you
...
export function isMondayTokenFromEnv(): boolean {
  return process.env.NODE_ENV === 'development' && Boolean(process.env.MONDAY_API_TOKEN?.trim())
}
