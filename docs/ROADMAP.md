...
- **Run `dispatchFormSync` outside the submission's transaction** — Raised 2026-07-27 while debugging a production 404 on `POST /api/form-submissions` for forms with an upload field. `dispatchFormSync` is an `afterChange` hook, so it shares the create's transaction: the hook passed a *populated* media document (REST default depth) as `id` into `payload.findByID`, the resulting SQL error aborted the transaction, the catch block's own `syncStatus: 'error'` write then failed inside that dead transaction, and Payload surfaced the whole thing to the visitor as a bare `404 Not Found` with the submission rolled back and lost. Both immediate bugs are fixed (id-vs-object resolution, and the status write is now non-fatal), so ordinary Monday/network failures behave correctly — submission saved, `syncStatus: 'error'`. What's still open is the structural hole: *any* future DB-level error inside the hook can still roll back the visitor's submission. Moving the dispatch to an `afterOperation` hook (or a queued job) that doesn't pass `req` would run it on its own connection after commit, making a sync failure structurally incapable of losing a lead. Touches the resync endpoint's assumptions too, so it deserves its own pass rather than a drive-by.
